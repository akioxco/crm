...
@section('content-header')
    <h1>
        {{ trans('pipelines::contacts.title.create contact') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.pipelines.contact.index') }}">{{ trans('pipelines::contacts.title.contacts') }}</a></li>
        <li class="active">{{ trans('pipelines::contacts.title.create contact') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.pipelines.contact.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('pipelines::admin.contacts.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
