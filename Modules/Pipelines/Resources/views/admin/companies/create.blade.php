...
@section('content-header')
    <h1>
        {{ trans('pipelines::companies.title.create company') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.pipelines.company.index') }}">{{ trans('pipelines::companies.title.companies') }}</a></li>
        <li class="active">{{ trans('pipelines::companies.title.create company') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.pipelines.company.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('pipelines::admin.companies.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
