...
@section('content-header')
    <h1>
        {{ trans('pipelines::emails.title.emails') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('pipelines::emails.title.emails') }}</li>
    </ol>
@stop
        ...
        <div class="col-xs-12">
            <div class="row">
                <div class="btn-group pull-right" style="margin: 0 15px 15px 0;">
                    <a href="{{ route('admin.pipelines.email.create') }}" class="btn btn-primary btn-flat" style="padding: 4px 10px;">
                ...
                <div class="box-body">
                    <div class="table-responsive">
                        <table class="data-table table table-bordered table-hover">
                            ...
                            <tr>
                                <td>
                                    <a href="{{ route('admin.pipelines.email.edit', [$email->id]) }}">
                                        {{ $email->created_at }}
                                    </a>
                                </td>
                                <td>
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('pipelines::emails.title.create email') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.pipelines.email.create') ?>" }
                ]
            });
        });
    </script>
