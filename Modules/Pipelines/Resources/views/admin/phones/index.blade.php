...
@section('content-header')
    <h1>
        {{ trans('pipelines::phones.title.phones') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('pipelines::phones.title.phones') }}</li>
    </ol>
@stop
        ...
        <div class="col-xs-12">
            <div class="row">
                <div class="btn-group pull-right" style="margin: 0 15px 15px 0;">
                    <a href="{{ route('admin.pipelines.phone.create') }}" class="btn btn-primary btn-flat" style="padding: 4px 10px;">
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('pipelines::phones.title.create phone') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.pipelines.phone.create') ?>" }
                ]
            });
        });
    </script>
